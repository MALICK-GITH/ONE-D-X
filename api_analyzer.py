"""
ONE-DELUX-FAST - Analyse approfondie de l'API 888starz
...
class APIAnalyzer:
    ...
    def test_api_performance(self) -> Dict[str, Any]:
        ...
        # Faire 5 requêtes pour tester la stabilité
        successes = 0
        for i in range(5):
            ...
            try:
                ...
                performance_metrics["response_times"].append(elapsed)
                
                if response.status_code == 200:
                    successes += 1
                    performance_metrics["data_size"] = len(response.content)
                    
            except Exception as e:
                performance_metrics["response_times"].append(-1)
                print(f"Erreur requête {i+1}: {e}")
            
            time.sleep(1)  # Attendre 1 seconde entre les requêtes
        
        performance_metrics["success_rate"] = (successes / 5) * 100
        valid_times = [t for t in performance_metrics["response_times"] if t > 0]
        if valid_times:
            performance_metrics["avg_response_time"] = sum(valid_times) / len(valid_times)
...
{'─' * 80}
• Taux de succès: {performance['success_rate']:.1f}%
• Temps de réponse moyen: {performance['avg_response_time']:.3f}s
• Taille des données: {performance['data_size'] / 1024:.2f} KB
• Temps de réponse individuels: {[f'{t:.3f}s' if t > 0 else 'FAIL' for t in performance['response_times']]}
...
2. PARALLÉLISME: Les requêtes sont rapides (~{performance['avg_response_time']:.2f}s), paralléliser possible
3. FILTRAGE: Utiliser les paramètres pour réduire la charge (sports, count)
4. ERROR HANDLING: Implement retry logic pour les échecs (taux: {100-performance['success_rate']:.1f}%)
5. DATA VALIDATION: Vérifier toujours 'Success' avant de traiter les données
6. TYPE DE PARIS PRÉFÉRENTIELS: Focus sur types 1,2,3,4 (1X2) et 9,10 (Over/Under)
...
def main():
    ...
    params = {
        'sports': '85', 'count': '40', 'lng': 'fr', 'gr': '789', 'mode': '4',
        'country': '96', 'partner': '233', 'getEmpty': 'true',
    ...
    try:
        ...
        print(f"Erreur lors de la sauvegarde des données brutes: {e}")
